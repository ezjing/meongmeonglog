...
export default function WalkFinishScreen() {
  ...
  const handleGoHomeLater = async () => {
    if (!activeWalk || cancelWalk.isPending) return;

    const confirmed = await showAlert({
      icon: "🐾",
      title: "정말 나갈까요?",
      message:
        "작성 중인 내용과 지금까지의 산책 기록이 모두 사라져요. 저장되지 않아요.",
      cancelLabel: "계속 작성",
      confirmLabel: "나가기",
      destructive: true,
    });
      ...
      >
        <View style={styles.summaryBar}>
          ...
          <Chip
            label={`⏱ ${formatDuration(
              frozenElapsedSec ??
                getElapsedSecFromStartedAt(activeWalk?.startedAt),
            )}`}
            selected
          />
          <Chip
        ...
        >
          <Text
            style={[
              styles.skipLink,
              cancelWalk.isPending && styles.skipLinkDisabled,
            ]}
          >
            그만두고 홈으로 돌아가기
          </Text>
